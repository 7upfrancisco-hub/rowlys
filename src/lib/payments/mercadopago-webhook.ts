...
import { fetchPaymentInfo, mapMpStatus, verifyWebhookSignature } from "@/lib/payments/mercadopago";
import { PHANTOM_CANCEL_REASON } from "@/lib/phantom-orders";

// Lógica compartida por las dos rutas de webhook de Mercado Pago:
//  - POST /api/webhooks/mercadopago         (locales con el token global)
//  - POST /api/webhooks/mercadopago/[tenant] (locales con su propia cuenta
//    conectada — el slug en la URL es lo que le permite a esa ruta resolver
//    QUÉ access token usar antes de llamar a fetchPaymentInfo)
// El único parámetro que varía entre ambas es `accessToken`; todo lo demás
// (verificar firma, mapear estado, actualizar Payment/Order, revivir
// pedidos fantasma) es idéntico — separado acá para no mantener dos copias.
//
// Se responde 200 siempre que la notificación se haya procesado (o se
// pueda ignorar): un no-2xx hace que MP reintente.
export async function handleMercadoPagoWebhook(
  ...
  let info;
  try {
    info = await fetchPaymentInfo(dataId, accessToken);
  } catch (err) {
